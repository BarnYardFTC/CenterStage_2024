...
public class RRRB extends LinearOpMode {
    AutonomousRB.spike_position position;
    ...
    public void runOpMode() {
        ...
        while (opModeInInit()) {
            if (PixelDetectorRB.getSpike_position() == 0) {
                position = AutonomousRB.spike_position.LEFT;
                HardwareLocal.green();
            }
            else if (PixelDetectorRB.getSpike_position() == 1) {
                position = AutonomousRB.spike_position.CENTER;
                HardwareLocal.green();
            }
            else {
                position = AutonomousRB.spike_position.RIGHT;
                HardwareLocal.blink();
            }
