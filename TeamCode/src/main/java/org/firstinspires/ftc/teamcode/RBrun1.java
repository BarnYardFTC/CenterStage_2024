package org.firstinspires.ftc.teamcode;

public class RBrun1 {

    static int phase = 1;

    static final int PHASE_1_POSITION = 800;
    static final int PHASE_2_POSITION = -100;
    static final int PHASE_3_POSITION = -100;
    static final int PHASE_4_POSITION = -50;
    static final int PHASE_5_POSITION = 1100;
    static final int PHASE_6_POSITION = -620;
    static final int PHASE_7_POSITION = 200;
    static final int PHASE_8_POSITION = -2200;
    static final int PHASE_10_POSITION = Arm.MINIMAL_HOLD_POSITION;
    static final int PHASE_11_POSITION = -1100;
    static final int PHASE_12_POSITION = -500;

}