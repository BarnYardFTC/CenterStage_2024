...
public class RRBB extends LinearOpMode {
    AutonomousBB.spike_position position;
    ...
    public void runOpMode() {
        ...
        while (opModeInInit()) {
            if (PixelDetectorBB.getSpike_position() == 0) {
                position = AutonomousBB.spike_position.LEFT;
                HardwareLocal.green();
            }
            else if (PixelDetectorBB.getSpike_position() == 1) {
                position = AutonomousBB.spike_position.CENTER;
                HardwareLocal.green();
            }
            else {
                position = AutonomousBB.spike_position.RIGHT;
                HardwareLocal.blink();
            }
